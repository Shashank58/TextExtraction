...
public class SVImageHandler {
   ...
   */
  public static PImage readImage(int size, java.io.BufferedReader in) {
    ...
    byte[] binarydata = javax.xml.bind.DatatypeConverter.parseBase64Binary(new String(charbuffer));
    // Convert the binary data to a byte stream and parse to image.
    java.io.ByteArrayInputStream byteStream = new java.io.ByteArrayInputStream(binarydata);
    try {
      PImage img = new PImage(javax.imageio.ImageIO.read(byteStream));
      return img;
    } catch (java.io.IOException e) {
